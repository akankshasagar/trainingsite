...
namespace CyberSecurity_new.Controllers
{
    ...
    [ApiController]
    public class AttackSurfacesAnswers001 : ControllerBase
    {
        private readonly AppDbContext _authContext;

        public AttackSurfacesAnswers001(AppDbContext authContext)
        {
            _authContext = authContext;
        }

        [HttpPost]
        public async Task<IActionResult> SubmitForm([FromBody] AttackSurfaces submission)
        {
            ...
            var existingSubmission = await _authContext.AttackSurfacesTest.FirstOrDefaultAsync(s => s.Email == userEmail);
            ...
            _authContext.AttackSurfacesTest.Add(submission);
            await _authContext.SaveChangesAsync();
            return Ok(new { Message = $"You Scored: {marks} out of 10" });
        }

        private int CalculateMarks(AttackSurfaces submission)
        {
            int marks = 0;
        ...
        [HttpGet]
        [Authorize]
        public async Task<ActionResult<AttackSurfaces>> GetAllResponses()
        {
            var userEmail = User.FindFirst(ClaimTypes.Email)?.Value;

            var userResponses = await _authContext.AttackSurfacesTest
        ...
        /*[HttpGet("test01")]
        [Authorize]*/
        [HttpPost("attacksurfacestest001")]
        [Authorize]
        public async Task<ActionResult<bool>> CheckEmailExists()
        {

            var userEmail = User.FindFirst(ClaimTypes.Email)?.Value;
            var emailExists = await _authContext.AttackSurfacesTest.AnyAsync(e => e.Email == userEmail);
            if (!emailExists)
            {
