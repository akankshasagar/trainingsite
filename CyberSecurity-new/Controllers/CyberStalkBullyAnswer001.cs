...
namespace CyberSecurity_new.Controllers
{
    ...
    [ApiController]
    public class CyberStalkBullyAnswer001 : ControllerBase
    {
        private readonly AppDbContext _authContext;

        public CyberStalkBullyAnswer001(AppDbContext authContext)
        {
            _authContext = authContext;
        }

        [HttpPost]
        public async Task<IActionResult> SubmitForm([FromBody] CyberStalkBullyTest001 submission)
        {
            ...
            var existingSubmission = await _authContext.CyberStalkBullyTest001s.FirstOrDefaultAsync(s => s.Email == userEmail);
            ...
            _authContext.CyberStalkBullyTest001s.Add(submission);
            await _authContext.SaveChangesAsync();
            return Ok(new { Message = $"You Scored: {marks} out of 10" });
        }

        private int CalculateMarks(CyberStalkBullyTest001 submission)
        {
            int marks = 0;
        ...
        [HttpGet]
        [Authorize]
        public async Task<ActionResult<CyberStalkBullyTest001>> GetAllResponses()
        {
            var userEmail = User.FindFirst(ClaimTypes.Email)?.Value;

            var userResponses = await _authContext.CyberStalkBullyTest001s
        ...
        /*[HttpGet("test01")]
        [Authorize]*/
        [HttpPost("cyberstalkbullytest001")]
        [Authorize]
        public async Task<ActionResult<bool>> CheckEmailExists()
        {

            var userEmail = User.FindFirst(ClaimTypes.Email)?.Value;
            var emailExists = await _authContext.CyberStalkBullyTest001s.AnyAsync(e => e.Email == userEmail);
            if (!emailExists)
            {
                return BadRequest(new { Message = "Please Submit The Test" });
            }
            return Ok(new { Message = "You have successfully completed the course" });
        }
    }
}
