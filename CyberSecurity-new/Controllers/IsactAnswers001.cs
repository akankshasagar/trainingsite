...
namespace CyberSecurity_new.Controllers
{
    ...
    [ApiController]
    public class IsactAnswers001 : ControllerBase
    {
        private readonly AppDbContext _authContext;

        public IsactAnswers001(AppDbContext authContext)
        {
            _authContext = authContext;
        }

        [HttpPost]
        public async Task<IActionResult> SubmitForm([FromBody] IsactTest001 submission)
        {
            ...
            var existingSubmission = await _authContext.IsactTest001s.FirstOrDefaultAsync(s => s.Email == userEmail);
            ...
            _authContext.IsactTest001s.Add(submission);
            await _authContext.SaveChangesAsync();
            return Ok(new { Message = $"You Scored: {marks} out of 10" });
        }

        private int CalculateMarks(IsactTest001 submission)
        {
            int marks = 0;
        ...
        [HttpGet]
        [Authorize]
        public async Task<ActionResult<IsactTest001>> GetAllResponses()
        {
            var userEmail = User.FindFirst(ClaimTypes.Email)?.Value;

            var userResponses = await _authContext.IsactTest001s
        ...
        /*[HttpGet("test01")]
        [Authorize]*/
        [HttpPost("isactest001")]
        [Authorize]
        public async Task<ActionResult<bool>> CheckEmailExists()
        {

            var userEmail = User.FindFirst(ClaimTypes.Email)?.Value;
            var emailExists = await _authContext.IsactTest001s.AnyAsync(e => e.Email == userEmail);
            if (!emailExists)
            {
