...
namespace CyberSecurity_new.Controllers
{
    ...
    [ApiController]
    public class PhishingSpoofingAnswer001 : ControllerBase
    {
        private readonly AppDbContext _authContext;

        public PhishingSpoofingAnswer001(AppDbContext authContext)
        {
            _authContext = authContext;
        }

        [HttpPost]
        public async Task<IActionResult> SubmitForm([FromBody] PhishingSpoofingTest001 submission)
        {
            ...
            var existingSubmission = await _authContext.PhishingSpoofingTest001s.FirstOrDefaultAsync(s => s.Email == userEmail);
            ...
            _authContext.PhishingSpoofingTest001s.Add(submission);
            await _authContext.SaveChangesAsync();
            return Ok(new { Message = $"You Scored: {marks} out of 10" });
        }

        private int CalculateMarks(PhishingSpoofingTest001 submission)
        {
            int marks = 0;
        ...
        [HttpGet]
        [Authorize]
        public async Task<ActionResult<PhishingSpoofingTest001>> GetAllResponses()
        {
            var userEmail = User.FindFirst(ClaimTypes.Email)?.Value;

            var userResponses = await _authContext.PhishingSpoofingTest001s
        ...
        /*[HttpGet("test01")]
        [Authorize]*/
        [HttpPost("phishingspoofingtest001")]
        [Authorize]
        public async Task<ActionResult<bool>> CheckEmailExists()
        {

            var userEmail = User.FindFirst(ClaimTypes.Email)?.Value;
            var emailExists = await _authContext.PhishingSpoofingTest001s.AnyAsync(e => e.Email == userEmail);
            if (!emailExists)
            {
