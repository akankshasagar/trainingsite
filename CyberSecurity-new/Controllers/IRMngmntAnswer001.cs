...
namespace CyberSecurity_new.Controllers
{
    ...
    [ApiController]
    public class IRMngmntAnswer001 : ControllerBase
    {
        private readonly AppDbContext _authContext;

        public IRMngmntAnswer001(AppDbContext authContext)
        {
            _authContext = authContext;
        }

        [HttpPost]
        public async Task<IActionResult> SubmitForm([FromBody] IRMngmntTest001 submission)
        {
            ...
            var existingSubmission = await _authContext.iRMngmntTest001s.FirstOrDefaultAsync(s => s.Email == userEmail);
            ...
            _authContext.iRMngmntTest001s.Add(submission);
            await _authContext.SaveChangesAsync();
            return Ok(new { Message = $"You Scored: {marks} out of 10" });
        }

        private int CalculateMarks(IRMngmntTest001 submission)
        {
            int marks = 0;
        ...
        [HttpGet]
        [Authorize]
        public async Task<ActionResult<IRMngmntTest001>> GetAllResponses()
        {
            var userEmail = User.FindFirst(ClaimTypes.Email)?.Value;

            var userResponses = await _authContext.iRMngmntTest001s
        ...
        /*[HttpGet("test01")]
        [Authorize]*/
        [HttpPost("irmngmnttest001")]
        [Authorize]
        public async Task<ActionResult<bool>> CheckEmailExists()
        {

            var userEmail = User.FindFirst(ClaimTypes.Email)?.Value;
            var emailExists = await _authContext.iRMngmntTest001s.AnyAsync(e => e.Email == userEmail);
            if (!emailExists)
            {
