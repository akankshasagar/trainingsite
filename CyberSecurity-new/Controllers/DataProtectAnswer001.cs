...
namespace CyberSecurity_new.Controllers
{
    ...
    [ApiController]
    public class DataProtectAnswer001 : ControllerBase
    {
        private readonly AppDbContext _authContext;

        public DataProtectAnswer001(AppDbContext authContext)
        {
            _authContext = authContext;
        }

        [HttpPost]
        public async Task<IActionResult> SubmitForm([FromBody] DataProtectTest001 submission)
        {
            ...
            var existingSubmission = await _authContext.DataProtectTest001s.FirstOrDefaultAsync(s => s.Email == userEmail);
            ...
            _authContext.DataProtectTest001s.Add(submission);
            await _authContext.SaveChangesAsync();
            return Ok(new { Message = $"You Scored: {marks} out of 10" });
        }

        private int CalculateMarks(DataProtectTest001 submission)
        {
            int marks = 0;
        ...
        [HttpGet]
        [Authorize]
        public async Task<ActionResult<DataProtectTest001>> GetAllResponses()
        {
            var userEmail = User.FindFirst(ClaimTypes.Email)?.Value;

            var userResponses = await _authContext.DataProtectTest001s
        ...
        /*[HttpGet("test01")]
        [Authorize]*/
        [HttpPost("dataprotecttest001")]
        [Authorize]
        public async Task<ActionResult<bool>> CheckEmailExists()
        {

            var userEmail = User.FindFirst(ClaimTypes.Email)?.Value;
            var emailExists = await _authContext.DataProtectTest001s.AnyAsync(e => e.Email == userEmail);
            if (!emailExists)
            {
